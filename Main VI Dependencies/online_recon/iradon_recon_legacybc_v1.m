function [IR_crop, proj_mag_crop_bc, theta] = iradon_recon(cropramppts,num_proj,RXmag,RXphase,angles,LPF_cut,enable_2nd_bc)
%UNTITLED4 Summary of this function goes here
%   Detailed explanation goes here
%%%% baseline correction used here uses the mean(first 4 pts of each) for each angle
...
% baseline_correction = mean(mean(projections_complex_crop(1:4,:))); %%%%%%%%%%%%%%%%%%%%%%% Legacy BC changed 9/16/21
baseline_correction = mean(projections_complex_crop(1:4,:));
%baseline_correction = 0+1i*0;

% proj_mag_crop_bc  = abs(projections_complex_crop-baseline_correction); %%%%%%%%%%%%%%%%%%%%%% Legacy BC changed 9/16/21
